...
import type { ComponentPreviewId, ResolvedButtonPreviewProps } from "./types";

interface GetComponentPreviewsOptions {
  compact: boolean;
  toggleState: boolean;
  setToggleState: Dispatch<SetStateAction<boolean>>;
  modalOpen: boolean;
  setModalOpen: Dispatch<SetStateAction<boolean>>;
  buttonProps: ResolvedButtonPreviewProps;
}
...
export function getComponentPreviews({
  ...
  setModalOpen,
  buttonProps,
}: GetComponentPreviewsOptions): Partial<Record<ComponentPreviewId, ReactNode>> {
  return {
    button: compact ? (
    ...
    toggle: compact ? (
      <CyberToggle checked={toggleState} onChange={setToggleState} label="Enable" />
    ) : (
      <div className="flex flex-col gap-4">
        <CyberToggle
          checked={toggleState}
    ...
    modal: (
      <div>
        {compact ? (
          ...
          <CyberButton onClick={() => setModalOpen(true)}>Open Modal</CyberButton>
        )}
        <CyberModal
          isOpen={modalOpen}
        ...
        <CyberTabs
          tabs={[
            ...
            },
            {
              ...
              content: (
                <div className="p-4">
                  <p className="text-sm font-mono">[2026-02-10] System initialized</p>
                </div>
              ),
            },
          ]}
    ...
    "cyber-grid": compact ? (
      ...
      <div className="relative w-full h-48 bg-gray-900 overflow-hidden">
        <CyberGrid />
        <div className="relative z-10 h-full flex items-center justify-center">
          <span className="text-2xl font-mono font-bold text-cyan-glow">CYBER GRID</span>
        </div>
      </div>
    ),
  };
